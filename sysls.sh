...
#Техническое задание
#
#    Приложение должно работать на любой системе, с любыми пользовательскими настройками в путях к интепретатору bash
#    Приложение должно иметь систему команд, реализованную через передачу аргументов. Команды должны иметь два вида:
...
#    Команды должны уметь принимать дополнительные аргументы, при этом если аргумент в команде не был указан приложение должно выдать какую-то полезную информацию для пользователя. Варианты применения:
...
#
#    Если команд не было указано, то нужно вывести usage который подскажет пользователю о том, как использовать это приложение. По сути вывести тоже что будет выведено при работе с --help.
#    В скрипте разрешено использование утилит из пакета sysstat, ifstat (для мониторинга нагрузки на сетевые интерфейсы) и предустановленных утилит free, lsblk, w и т.п.
#    Приложение должно уметь обрабатывать ошибки.
...
#    В коде приложения обязательно должны использоваться if, for, for in и case (каждая не менее 1 раза).
